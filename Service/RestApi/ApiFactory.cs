...
namespace Service.RestApi
{
    ...
    public class ApiFactory
    {
        ...
        public DateTime CalculateEndTerm(int idUser, int numberOfDays, DateTime startDate)
        {
                ...
                using (HttpWebResponse response = (HttpWebResponse)request.GetResponse())
                {
                    using (Stream responseStream = response.GetResponseStream())
                    {
                        using (StreamReader responseReader = new StreamReader(responseStream))
                        {
                            ...
                            var termR = JObject.Parse(resultJson);
                            var x = (termR["dateEnd"].ToString());
                            var epoch = new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc);
                            var y = epoch.AddMilliseconds(Double.Parse(x));
                            return termR.HasValues ? y : startDate;
                        }
                    }
                }
            }
