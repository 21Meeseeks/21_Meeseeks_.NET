    ...
    public class RequestController : Controller
    {
        ...
        public async System.Threading.Tasks.Task<ActionResult> Index()
        {
            ...
            if (Session["role"] == "Client")
            {
                ...
                var R = await HTTPC.PostAsJsonAsync<Client>("Request/client", C);
                ViewBag.result = R.Content.ReadAsAsync<IEnumerable<ProjectRequest>>().Result;
            }
            else
            {
                HttpResponseMessage HTTPRM = HTTPC.GetAsync("Request").Result;
                if (HTTPRM.IsSuccessStatusCode || HTTPRM.StatusCode == System.Net.HttpStatusCode.Found)
                {
        ...
        public async System.Threading.Tasks.Task<ActionResult> Details(int ID)
        {
            ...
            ViewBag.dateStartProject = ConvertDate(ViewBag.result.dateStartProject.ToString());
            ViewBag.dateEndProject = ConvertDate(ViewBag.result.dateEndProject.ToString());
            return View("Details");
        }
        ...
        public ActionResult Create()
        {
            ...
            HTTPC.BaseAddress = new Uri("http://localhost:18080/21meeseeks-web/rest/");
            HTTPC.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
            HttpResponseMessage HTTPRM = HTTPC.GetAsync("competence").Result;
            if (HTTPRM.IsSuccessStatusCode || HTTPRM.StatusCode == System.Net.HttpStatusCode.Found)
            {
        ...
        public async System.Threading.Tasks.Task<ActionResult> Update(int ID)
        {
            ...
            HttpResponseMessage HTTPRM = HTTPC0.GetAsync("competence").Result;
            if (HTTPRM.IsSuccessStatusCode || HTTPRM.StatusCode == System.Net.HttpStatusCode.Found)
            {
                ViewBag.result = HTTPRM.Content.ReadAsAsync<IEnumerable<Competence>>().Result;
        ...
        public async System.Threading.Tasks.Task<ActionResult> IndexLeaveRequest()
        {
            HttpClient HTTPC = new HttpClient();
            HTTPC.BaseAddress = new Uri("http://localhost:18080/21meeseeks-web/rest/");
            HTTPC.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
            if (Session["role"] == "Resource")
            {
                ...
                var Rsp = await HTTPC.PostAsJsonAsync<Resource>("Request/Leave/resource", R);
                ViewBag.result = Rsp.Content.ReadAsAsync<IEnumerable<LeaveRequest>>().Result;
            }
            else
            {
